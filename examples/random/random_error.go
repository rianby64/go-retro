...
func main() {
	for i := 0; i < 100; i++ {
		...
		err := retro.DoWithRetry(func() error {
			value, err := genValue()
			var msg string
			msg = fmt.Sprintf("%s try %dth chance.", msg, i)
			if err != nil {
				msg = fmt.Sprintf("%s Value %d is below the given threshold", msg, value)
				if i == maxAttemps {
					msg = fmt.Sprintf("%s and reached maxAttemps", msg)
				}
			} else {
				msg = fmt.Sprintf("%s Value %d above the threshold - OK", msg, value)
